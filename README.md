...
The original mascot is a single PNG with its background baked in. The hero uses a custom SVG interpretation with independent `bin-body` and `bin-lid` groups; the original mascot remains in the logo and resident app. The hero pairs the continuous lowercase `trashbuddy` wordmark with the unchanged silver SVG. The entrance runs once per landing-route mount: the bin rises into an 11° right lean with a -12° perspective turn, then the lid closes flush in 0.55 seconds, finishing at 1.5 seconds. All motion uses non-spring easing without overshoot. The ground shadow is independent of the raised bin. Reduced motion uses only a fade with no tilt. Other site branding is intentionally unchanged.
